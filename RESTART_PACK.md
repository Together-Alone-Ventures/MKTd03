...
DELETION-STATE MATERIAL VALIDATION SUMMARY:

S7-14 added a standalone private structural validator for `DeletionStateMaterial`.

S7-14 accepts only `TombstonedPosition(vec![0x01])`, rejects all `EmptyPosition(_)`, and rejects all other tombstoned payloads.
...
RECEIPT DIRECTION-CONSISTENCY SUMMARY:

S7-17 wires direction-vs-record-position consistency into `validate_receipt(receipt: &Receipt)` after the S7-16 structural pre-check.

S7-17 derives `record_position_key` through the non-trapping crate-internal Result path, defensively reparses the proof envelope, and then calls the existing S7-13 direction validator.

S7-17 still leaves downstream proof, certification, version, and receipt semantics unimplemented after the direction check passes.

No `.did`, Cargo, docs/spec, docs/test-vectors, fixture, public API, hashing/preimage/tag bytes, or `leaf_hash.rs` changes were made in S7-17.
...
The next slice is TBD / await G+C scoping.
