...
The authority map may be updated only as part of the current phase deliverables and must not be used to override reviewed ADR or interface content without explicit G approval.
...
- concepts that remain valid and are revalidated against ADR-00, ADR-01, ADR-02, and ADR-03,
- lifecycle/state-management ideas that remain valid and are revalidated against the current ADR set,
- diagnostics/status expectations that carry forward only where consistent with the current ADR set,
- compatibility/versioning expectations that carry forward only where consistent with the current ADR set,
- reusable or likely reusable protocol machinery, subject to the Phase 4 reuse audit.
...
No open question may remain by Phase 6 if it could change public types, verifier inputs, hashing/encoding rules, adapter obligations, lifecycle semantics, or version behavior.
...
This audit must explicitly state the exact MKTd02 repository commit or tag under review, and the exact scope of modules included and excluded.

For each meaningful MKTd02 module in that anchored scope, classify it as:
...
- whether a later separate MKTd02 evaluation may be warranted,
...
- Normative notes, interface rules, vectors, and fixtures must not use TinyPress-shaped payloads, route names, schema names, canister names, or fixture examples.
...
- interface-level field optionality rules,
- default-value interpretation rules,
- version-tag interpretation rules,
...
A fixture manifest must exist at:

`docs/test-vectors/fixtures/manifest.md`

At minimum the manifest must define:

- fixture naming convention,
- positive vs negative fixture naming split,
- version-tagging convention,
- encoding convention,
- how each fixture maps to the human-readable vector note or negative-case note.
...
- every meaningful MKTd02 module in the anchored review scope is classified,
- reuse audit names the exact MKTd02 commit or tag reviewed,
...
- fixture manifest exists and is reviewed,
- no open question remains that could change public types, verifier inputs, hashing/encoding rules, adapter obligations, lifecycle semantics, or version behavior,
- authority map is updated,
...
1. fresh MKTd03 implementation of approved reusable abstractions where justified,  
...
- Create machine-readable fixtures and manifest.
